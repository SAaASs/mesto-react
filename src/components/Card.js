...
export function Card({card, clickHandler, likeHandler, onDeletePopupOpen}) {
    ...
    const cardLikeButtonClassName = ( 
        `element__bottom-like ${isLiked && 'element__bottom-like_liked'}` 
      );; 
    const isBelongToCurrentUser = user?._id === card?.owner?._id
    return (
        <div className="element">
        {isBelongToCurrentUser && <button onClick={onDeletePopupOpen} data-id="" type="button" className="element__delete"></button>}
        ...
        <div className="element__bottom">
            ...
            <div className="element__bottom-like-container">
                <button type="button" data-id="full-image-popup" onClick={likeHandler} className={cardLikeButtonClassName}></button>
